...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_maukas_lounas_ei_vie_negatiiviseksi(self):
        kortti = Maksukortti(200)
        kortti.syo_maukkaasti()
        self.assertEqual(str(kortti), "Kortilla on rahaa 2.00 euroa")
    
    def test_kortille_ei_ladata_negatiivisia(self):
        self.kortti.lataa_rahaa(-200)
        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")
    
    def test_kortilla_voi_ostaa_edullisesti_tasarahalla(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()
        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")
    
    def test_kortilla_voi_ostaa_maukkaasti_tasarahalla(self):
